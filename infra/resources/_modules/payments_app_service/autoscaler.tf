module "payments_app_service_autoscaler" {
  ...
  target_service = {
    app_service_name = module.payments.app_service.app_service.name
  }

  scheduler = {
    maximum = local.payments.autoscale_settings.max
    normal_load = {
      default = local.payments.autoscale_settings.default
      minimum = local.payments.autoscale_settings.min
    }
  }
